...
#define _min(a,b) (((a) < (b)) ? (a) : (b))
    ...
    if( !GetWindowSizeHints( tmp_win ) ){
	    fprintf( stderr, "ConstrainSize(): can't obtain XSizeHints, not attempting to constrain (re)size operation\n" );
	    return;
    }
    ...
    fprintf( stderr, "dwidth,dheight= %d,%d\n", dwidth, dheight );
    ...
    fprintf( stderr, "minWidth,minHeight=%d,%d baseWidth,baseHeight=%d,%d maxWidth,maxHeight=%d,%d xinc,yinc=%d,%d\n",
		minWidth, minHeight,
...
#define minAspectY tmp_win->hints.min_aspect.y
	...
	fprintf( stderr, "minAspectX,Y=%d,%d maxAspectX,Y=%d,%d\n",
		minAspectX, minAspectY,
 ...
 *         tmp_win - the TwmWindow pointer
 *
 * 20050105 RJVB: Xinerama support.
 *
 **********************************************************************
 */
...
unsigned int theDisplayWidth( int x_centre, int y_centre, int *base_x, int *base_y, int *head )
{ int i;
  long dist, min_dist;
  int best= 0;
	if( Scr->NumHeads ){
		   ...
		   */
		min_dist= dist= abs( (Scr->HeadInfo[0].x_org + Scr->HeadInfo[0].width/2 - x_centre) +
			(Scr->HeadInfo[0].y_org + Scr->HeadInfo[0].height/2 - y_centre) );
		for( i= 1; i< Scr->NumHeads; i++ ){
			dist= abs( (Scr->HeadInfo[i].x_org + Scr->HeadInfo[i].width/2 - x_centre) +
				(Scr->HeadInfo[i].y_org + Scr->HeadInfo[i].height/2 - y_centre) );
			if( dist< min_dist ){
				min_dist= dist;
				best= i;
			}
		...
		if( head ){
			*head= Scr->HeadInfo[best].screen_number;
		}
#ifdef DEBUG
		theHead= Scr->HeadInfo[best].screen_number;
#endif
		if( base_x ) *base_x= Scr->HeadInfo[best].x_org;
		if( base_y ) *base_y= Scr->HeadInfo[best].y_org;
		return( (unsigned int) Scr->HeadInfo[best].width );
	}
#if 0
	  /* Find the head that the window's current top-left co-ordinate is on, and
	   \ return the head's width and offset on the 'total screen'.
	   */
	for( i= 0; i< Scr->NumHeads; i++ ){
		if( x_centre>= Scr->HeadInfo[i].x_org && x_centre< Scr->HeadInfo[i].x_org+Scr->HeadInfo[i].width
			&& y_centre>= Scr->HeadInfo[i].y_org && y_centre< Scr->HeadInfo[i].y_org+Scr->HeadInfo[i].height
		){
			if( head ){
				*head= Scr->HeadInfo[i].screen_number;
			}
			if( base_x ) *base_x= Scr->HeadInfo[i].x_org;
			if( base_y ) *base_y= Scr->HeadInfo[i].y_org;
			return( (unsigned int) Scr->HeadInfo[i].width );
		}
	}
#endif
	if( head ){
	...
	if( base_x ) *base_x= 0;
	if( base_y ) *base_y= 0;
	return( (unsigned int) Scr->MyDisplayWidth );
}

unsigned int theDisplayHeight( int x_centre, int y_centre, int *base_x, int *base_y, int *head )
{ int i;
  long dist, min_dist;
  int best= 0;
	if( Scr->NumHeads ){
		...
		if( base_x ) *base_x= Scr->HeadInfo[best].x_org;
		if( base_y ) *base_y= Scr->HeadInfo[best].y_org;
		return( (unsigned int) Scr->HeadInfo[best].height );
	}
#if 0
	for( i= 0; i< Scr->NumHeads; i++ ){
		if( x_centre>= Scr->HeadInfo[i].x_org && x_centre< Scr->HeadInfo[i].x_org+Scr->HeadInfo[i].width
			...
			if( base_x ) *base_x= Scr->HeadInfo[i].x_org;
			if( base_y ) *base_y= Scr->HeadInfo[i].y_org;
			return( (unsigned int) Scr->HeadInfo[i].height );
		}
	}
#endif
	if( head ){
	...
	if( base_x ) *base_x= 0;
	if( base_y ) *base_y= 0;
	return( (unsigned int) Scr->MyDisplayHeight );
}

#else
#	define theDisplayWidth(a,b,c,d,e)	Scr->MyDisplayWidth
#	define theDisplayHeight(a,b,c,d,e)	Scr->MyDisplayHeight
#endif

#define TheDisplayWidth	theDisplayWidth(dragx+dragWidth/2,dragy+dragHeight/2,&basex,&basey,NULL)
#define TheDisplayHeight	theDisplayHeight(dragx+dragWidth/2,dragy+dragHeight/2,&basex,&basey,NULL)
...
int flag;
{
	...
	basey = 0;
	  ...
	  /* Get the head's or screen's size, and possibly the head's offset on the "total screen".
	   \ The offset is stored in the basex,basey variables by the macros defined just above.
	   */
	Width= TheDisplayWidth;
	Height= TheDisplayHeight;
#ifdef DEBUG
	fprintf( stderr, "%dx%d+%d+%d (bw=%d bw3D=%d titleH=%d) -> head #%d::%dx%d+%d+%d \"%s\"\n",
		dragWidth, dragHeight, dragx, dragy,
		tmp_win->frame_bw, tmp_win->frame_bw3D, tmp_win->title_height,
		theHead, Width, Height, basex, basey,
		tmp_win->name
	);
#endif
	...
	if( (frame_bw_times_2 = 2*tmp_win->frame_bw) > dragWidth ){
		 /* RJVB */
		frame_bw_times_2 = dragWidth;
	}
	else if( frame_bw_times_2> dragHeight ){
		 /* RJVB */
		frame_bw_times_2= dragHeight;
	}
        ...
        case ZOOM_NONE:
            break;
        case F_ZOOM:
            dragHeight = Height - frame_bw_times_2;
            dragy=basey;
            break;
        case F_HORIZOOM:
            dragx = basex;
            dragWidth = Width - frame_bw_times_2;
            break;
        case F_FULLZOOM:
            dragx = basex;
            dragy = basey;
        ...
        case F_LEFTZOOM:
            ...
            dragy = basey;
            dragHeight = Height - frame_bw_times_2;
            dragWidth = Width/2 - frame_bw_times_2;
            break;
        case F_RIGHTZOOM:
            dragx = basex + Width/2;
            dragy = basey;
        ...
        case F_TOPZOOM:
            dragx = basex;
            dragy = basey;
            dragHeight = Height/2 - frame_bw_times_2;
            dragWidth = Width - frame_bw_times_2;
            break;
        case F_BOTTOMZOOM:
            dragx = basex;
            dragy = basey + Height/2;
            dragHeight = Height/2 - frame_bw_times_2;
            dragWidth = Width - frame_bw_times_2;
            break;
         }
...
#ifdef DEBUG
	fprintf( stderr, "Initial new size %dx%d; ", dragWidth, dragHeight );
#endif
    ConstrainSize(tmp_win, &dragWidth, &dragHeight);
#ifdef DEBUG
    	fprintf( stderr, "constrained: %dx%d\n", dragWidth, dragHeight );
#endif
