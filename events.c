...
HandleExpose()
{
	...
	{
#ifdef I18N
	    XmbDrawString(dpy, Scr->InfoWindow, Scr->InfoFont->font_set,
			  Scr->NormalGC, 5,
			  (i*height) + Scr->InfoFont->y + 5,
			  Info[i], strlen(Info[i]));
#else
	    XDrawString(dpy, Scr->InfoWindow, Scr->NormalGC,
		5, (i*height) + Scr->InfoFont->y + 5, Info[i], strlen(Info[i]));
#endif	    
	}
