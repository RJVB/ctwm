...
#pragma GCC set_debug_pwd "/Users/bertin/work/src/ctwm-3.5"
...
# operating system:  Darwin (6.8.0)

# $XFree86: xc/config/cf/darwinLib.rules,v 1.5 2002/07/17 01:06:04 torrey Exp $
...
CXXDEPENDINCLUDES =
 CXXEXTRA_DEFINES =
CXXEXTRA_INCLUDES =
   CXXSTD_DEFINES = -D__powerpc__ -D__DARWIN__                                 -DNO_ALLOCA -DX_LOCALE -DCSRG_BASED $(CXXPROJECT_DEFINES)
       CXXOPTIONS =
...
MODLDCOMBINEFLAGS = -X -r
     ...
     STD_INCLUDES =
  STD_CPP_OPTIONS =
  STD_CPP_DEFINES =   -D__powerpc__ -D__DARWIN__                                 -DNO_ALLOCA -DX_LOCALE -DCSRG_BASED $(PROJECT_DEFINES)
      STD_DEFINES = -D__powerpc__ -D__DARWIN__                                 -DNO_ALLOCA -DX_LOCALE -DCSRG_BASED $(PROJECT_DEFINES)
 EXTRA_LOAD_FLAGS =
  EXTRA_LDOPTIONS =
  EXTRA_LIBRARIES =
             TAGS = ctags
...
#
 ...
 SOUND_INCLUDES =
     ...
     SESSIONOBJ = session.o
      X11R6DEFS = -DX11R6 -D_HAVE_XINERAMA_

     I18N_DEFS = -DI18N

LOCAL_LIBRARIES = $(M4LIB) $(IMCONVLIB) $(XPMLIB) $(XMULIB) $(XTOOLLIB) $(XLIB) $(SOUND_LIBS) $(XINERAMALIB)
...
libctwm.a:  libctwm.o $(EXTRALIBRARYDEPS)
	$(RM) $@
	$(AR) $@  libctwm.o
	$(RANLIB) $@
	$(_NULLCMD_)
...
all:: ctwm.$(MANSUFFIX).html

ctwm.$(MANSUFFIX).html: ctwm.$(MANNEWSUFFIX)
	$(RM) ctwm.$(MANSUFFIX).html ctwm.$(MANSUFFIX)-html
	$(RMAN) $(RMANOPTIONS) < ctwm.$(MANNEWSUFFIX) \
	  > ctwm.$(MANSUFFIX)-html && $(MV) ctwm.$(MANSUFFIX)-html $@

install.man:: ctwm.$(MANSUFFIX).html
	...
	@(SUF=`expr $(MANSUFFIX) \: '\(.\)'`; \
	 set -x; \
	 $(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) ctwm.$(MANSUFFIX).html $(DESTDIR)$(DOCHTMLDIR)/ctwm.$$SUF.html)

cleandir::
	$(RM) ctwm.$(MANSUFFIX).html ctwm.$(MANSUFFIX)-html

all:: ctwm.$(MANNEWSUFFIX)

ctwm.$(MANNEWSUFFIX): ctwm.$(MANSRCSUFFIX)
	$(RM) $@
	cd `dirname ctwm` && \
		$(LN) `basename ctwm.$(MANSRCSUFFIX)` `basename $@`

cleandir::
	$(RM) ctwm.$(MANNEWSUFFIX)

install.man:: ctwm.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) ctwm.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/ctwm.$(MANSUFFIX)
...
install:: system.ctwmrc
	@if [ -d $(DESTDIR)$(TWMDIR) ]; then \
		set +x; \
	else \
		if [ -h $(DESTDIR)$(TWMDIR) ]; then \
			(set -x; rm -f $(DESTDIR)$(TWMDIR)); \
		fi; \
		(set -x; $(MKDIRHIER) $(DESTDIR)$(TWMDIR)); \
	fi
	$(INSTALL) $(INSTALLFLAGS) $(INSTDATFLAGS) system.ctwmrc $(DESTDIR)$(TWMDIR)/system.ctwmrc

install:: xpm
	@if [ -d $(DESTDIR)$(TWMDIR)/images ]; then \
		set +x; \
	else \
		if [ -h $(DESTDIR)$(TWMDIR)/images ]; then \
			(set -x; rm -f $(DESTDIR)$(TWMDIR)/images); \
		fi; \
		(set -x; $(MKDIRHIER) $(DESTDIR)$(TWMDIR)/images); \
	fi
	@case '${MFLAGS}' in *[i]*) set +e;; esac; 	for i in xpm/?* ; do 		(set -x; $(INSTALL) -c $(INSTLIBFLAGS) $$i $(DESTDIR)$(TWMDIR)/images); 	done
...
deftwmrc.c:  system.ctwmrc
	...
	echo '' >>$@
	echo 'char *defTwmrc[] = {' >>$@
	sed -e '/^#/d' -e 's/"/\\"/g' -e 's/^/    "/' -e 's/$$/",/' 		system.ctwmrc >>$@
